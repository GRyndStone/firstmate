...
# hand-edit backlog files too, and primary config/no-mistakes-generation pins the
# same no-mistakes runtime generation for secondmate-spawned crewmates).
#
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend no-mistakes-generation}"

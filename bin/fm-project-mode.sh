...
#   - <name> - <desc> (added <date>)                  -> direct-PR off  (legacy / omitted mode)
#   - <name> [<mode>] - <desc> (added <date>)          -> <mode> off
#   - <name> [<mode> +yolo] - <desc> (added <date>)    -> <mode> on
#
# mode = how a finished change reaches main:
#   direct-PR    push + PR via gh-axi, focused tests/lint, no pipeline (default)
...
# mode falls back to "direct-PR off" and warns to stderr. no-mistakes is never
# the implicit default; a project must name [no-mistakes] explicitly.

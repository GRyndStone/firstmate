...
Append a fresh \`working:\` line only when substantive same-pane recovery begins after the run is terminal.
Any validation after recovered work must start a new no-mistakes run; do not reuse or continue the terminal run.

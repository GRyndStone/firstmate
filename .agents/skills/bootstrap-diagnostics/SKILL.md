...
- `TASKS_AXI: available` - a default-backend capability fact, not a problem; record it silently and use `AGENTS.md` section 10 for backlog mutations.
  ...
  If `tasks-axi` is missing, incompatible, or suppressed by `config/backlog-backend=manual`, use only the exclusive single-owner manual fallback owned by `docs/configuration.md` "Backlog backend"; concurrent-agent mutations wait for the wrapper.

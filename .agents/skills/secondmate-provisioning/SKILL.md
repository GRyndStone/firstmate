...
When safe, teardown closes the direct report's recorded backend endpoint only after exact-home ownership preflight, proves it absent, then removes the `data/secondmates.md` route, clears the main-home metadata, and removes the retired secondmate home.
...
With `--force`, teardown is the explicit discard path, but it first audits every child-home endpoint and refuses the entire retirement on an ownership anomaly or unavailable exact inventory before closing anything.
After that preflight succeeds, it closes the recorded child endpoints through their backends, discards child work and state inside the secondmate home, removes the route, releases the lease, and removes the retired secondmate home.

...
When `config/crew-dispatch.json` is active, its default candidate set self-routes ordinary crewmate and scout dispatch through the usage-burndown engine.
Optional natural-language rules can support an intentional task-specific override, whose concrete routing axes require an explicit reason.
...
Inheritance also copies the literal `config/crew-dispatch.json` file, so secondmates apply the same default self-routing candidate set for their own crewmates.
...
| agy | `--model <model>` | `--effort <low\|medium\|high>` only when the model token does not already end in `-low`, `-medium`, or `-high` | Verified on agy 1.1.7. Live launch probes, not `agy models`, are the authority for launchability. The baked effort suffix wins when both `--model` and `--effort` are supplied. Use `gemini-3.1-pro-low` for the current best CLI-reachable Pro-tier profile. `gemini-3.1-pro-high` is advertised but live agy warns it is no longer available and opens Gemini 3.6 Flash High instead. |
...
Model catalog and quota-accounting policy: `agy models` is not reliable evidence that a token can launch. It still listed `gemini-3.1-pro-high` after live agy warned that token was unavailable and silently opened Gemini 3.6 Flash High. Re-probe agy model tokens with a live idle launch/footer before treating any catalog token as routable; if a configured token later retires, `fm-spawn` checks the launch pane for the unavailable-model warning and refuses the spawn rather than reporting a downgraded Flash session as successful. The catalog also lists `claude-sonnet-4-6`, `claude-opus-4-6-thinking`, and `gpt-oss-120b-medium`; do not route those through antigravity because that would spend Google quota on model families already metered under the Claude/Codex providers and corrupt firstmate's usage-burndown accounting.

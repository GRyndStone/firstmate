...
The session-start secondmate liveness sweep uses a deeper `fm_backend_agent_alive` probe where verified, and the watcher's bounded paused-recheck death probe reuses the same primitive for paused crew endpoints (`docs/architecture.md` "Event-driven supervision").
Today that probe can classify tmux and herdr endpoints as `alive`, `dead`, or `unknown`; zellij, Orca, and cmux report `unknown` until their own agent-process classifiers are verified, and an `unknown` never counts as dead.
...
An absent `quota-axi` reports `MISSING: quota-axi (install: npm install -g quota-axi)`; at admission time, unreadable metered usage evidence is a loud error (`dispatch_error=usage-evidence-unreadable`, exit 70 when no live candidate remains) rather than a silent first-profile retain (see `bin/fm-dispatch-select.sh` and `docs/usage-burndown-dispatch.md`).
Being present is not the same as being current. The declared incorporation inventory in `deps/incorporations.conf` records what the first mate incorporates and what each piece is relied upon to do, and the same session-start path reports `DEPS:` lines when a component has fallen behind its published release, when the captain's own clone of it has moved ahead, or when an installed component no longer honors a contract the first mate parses - and stays silent when everything is current.
`docs/dependency-currency.md` owns that system: the inventory and contract formats, which components carry pinned contracts and why the rest do not, the offline and no-registry behavior, and the approved-upgrade and rollback commands.
...
FM_BACKEND_HERDR_COMPOSER_LINES=20  # herdr-only: tail lines scanned by the affirmative-empty pre-injection composer guard; prompt submission is atomic
...
FM_BACKEND_HERDR_PROMPT_TIMEOUT_MS=7000  # herdr-only: server-owned atomic agent-prompt wait bound; agent_prompt_stalled remains a distinct ineffective-submission verdict
...
FM_RECONCILE_MAX_WORKERS=8   # maximum concurrent per-task reconciliation observers in one watcher poll
FM_CODEX_WATCH_CHECKPOINT=180   # seconds per optional foreground watcher diagnostic checkpoint
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before an unchanged stale pane revalidates positive working evidence; current run-step/busy-pane/progressing-owned-command evidence stays quiet, while loss of it can escalate; also bounds the paused-recheck cadence of the watcher's agent-dead liveness probe
FM_OWNED_COMMAND_PROGRESS_GRACE=300 # seconds an unchanged exact-pid descendant-tree signature remains positive working evidence; captured into each register-command record
FM_PAUSE_RESURFACE_SECS=3600       # seconds before an idle absorbable park - a declared external wait, or the watcher's anchored green-run merge park - re-surfaces for a recheck in the watcher or away-mode daemon
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive possible-wedge escalations after positive working evidence disappears before demand-deep-inspection is added
FM_TEARDOWN_TOMBSTONE_SECS=120     # seconds a fresh state/<id>.tearing-down teardown tombstone lets the watcher absorb that task's gone endpoint before failing back to the endpoint-gone wake
...
FM_BUSY_REGEX='esc (to )?interrupt|esc to cancel|Working\.\.\.|Ctrl\+c:cancel'   # busy-pane signatures, shared by watcher, fm-crew-state pane fallback, and tmux helper

...
An unchanged pane with current positive run-step, busy-pane, or progressing owned-command evidence remains quiet when `FM_STALE_ESCALATE_SECS` elapses and starts a fresh bounded recheck; repeated possible-wedge escalations only after that evidence disappears add an escalation count and, at `FM_WEDGE_DEMAND_INSPECT_COUNT`, a `demand-deep-inspection` marker.
...
When a crew or scout capture fails and the read-only tri-state observation (`fm_backend_target_state`, which never starts a server) positively returns `dead`, the watcher surfaces an immediately actionable `endpoint-gone` stale wake within one poll, regardless of any declared `paused:` wait and regardless of away mode, deduplicated by a per-endpoint marker until the endpoint reads alive again.
An `unknown` observation is unreadable evidence, never death proof, so it refuses the death classification and waits for a later readable observation.
...
Non-paused secondmate endpoints get the same per-poll tri-state observation and the same `endpoint-gone` wake on positive death: a persistent supervisor whose endpoint is proven absent between session starts must wake the primary immediately rather than waiting for the next session-start liveness sweep, because a confirmed missing endpoint is death, not the healthy idle pane the secondmate stale exemption protects.
...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only when `bin/fm-crew-state.sh` reports positive evidence that the crew is still working: an actively running no-mistakes step for that crew's branch, a task-scoped identity-bound command with fresh descendant progress, or a backend busy signature.
A crew that declares `paused:` for a known external wait is separately absorbed while idle and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge, once any machine-observable completion condition is registered.
An observable wait without a predicate, tracked-process identity, backend completion signal, or legacy check produces one immediate fail-loud reconciliation wake instead of an indefinite stale label.
...
Run-step precedence still ejects the park for every non-absorbable verdict, so a failed or gate-parked run behind a stale `paused:` line surfaces immediately, and a busy pane or progressing owned command keeps outranking the declared pause.
...
Fresh stale panes use the same current-state read before trusting the status log, so an active run, busy pane, or progressing owned command outranks an old captain-relevant status-log line left behind before validation.
...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable review: it first consumes the watcher's fresh endpoint-matched reconciled observation, then attributes the matching no-mistakes run, active or terminal, to the crew's own branch and keeps that run-step authoritative even if the pane has closed.
...
For herdr, that no-run current-state path treats native `busy` and `idle` verdicts as authoritative, while only a native `unknown` verdict falls back to bounded pane capture.
For whole-fleet read-only review, `bin/fm-fleet-snapshot.sh --json` emits schema `fm-fleet-snapshot.v1` from the backlog, task metadata, current reconciled state and freshness, prior observed state, last status event sequence, external-wait registration, endpoint probes, PR/report pointers, scout reports, and secondmate return-channel guidance.
...
On every verified primary harness, tracked hook integration gives the primary session a push-based backstop: when work is in flight and no identity-matched watcher lock with a fresh beacon is live, direct Stop hooks block and passive turn-end hooks force bounded follow-ups until the shared predicate passes.

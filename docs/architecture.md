...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, check-script output such as PR merge polling or an X-mode mention, stale panes whose crew is neither provably working nor in an absorbable park whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, absorbable parks that remain idle past `FM_PAUSE_RESURFACE_SECS`, gone or confidently-dead crew endpoints, and heartbeat backstop hits.
...
A recorded endpoint that no longer exists is classified as death, not quiet (the 2026-07-12 incident: a crew died mid-`paused:` wait, its gone pane made every capture fail, and the watcher silently skipped it for hours).
When a crew or scout capture fails and the read-only existence probe (`fm_backend_target_exists`, which never starts a server) confirms the endpoint is gone, the watcher surfaces an immediately actionable `endpoint-gone` stale wake within one poll, regardless of any declared `paused:` wait and regardless of away mode, deduplicated by a per-endpoint marker until the endpoint reads alive again.
A normal teardown's own endpoint kill is not a death: `bin/fm-teardown.sh` touches a `state/<id>.tearing-down` tombstone before its endpoint-affecting cleanup and removes it with the task's other state files, and the watcher absorbs that task's gone endpoint only while the tombstone is fresh (`FM_TEARDOWN_TOMBSTONE_SECS`, default 120s) or the meta is already removed, failing back to the `endpoint-gone` wake once the bound lapses so a crashed teardown cannot re-open the original blind spot.
Non-paused secondmate endpoints get the same per-poll existence probe and the same `endpoint-gone` wake: a persistent supervisor whose endpoint vanishes between session starts must wake the primary immediately rather than waiting for the next session-start liveness sweep, because a missing endpoint is death, not the healthy idle pane the secondmate stale exemption protects.
The session-start sweep remains the respawn owner for secondmates; the wake only tells the primary to run that recovery now.
On the bounded paused-recheck cadence (never every cheap poll), a parked crew - a declared pause, or an anchored green-run merge park - whose endpoint still exists is additionally probed with `fm_backend_agent_alive`: a confident `dead` verdict surfaces an `agent-dead` stale wake once per death, while `alive` and every ambiguous or errored (`unknown`) read never count as dead - the same fail-closed principle as the secondmate liveness sweep and the herdr husk classifier.
The `agent-dead` wake fires regardless of away mode, on the same bounded cadence, so the away-mode daemon receives the explicit death verdict instead of a plain stale it might re-absorb as a declared pause; an ambiguous read during away mode still hands off only the plain stale.
The once-per-death `agent-dead` marker resets only on positive liveness evidence - a genuinely busy pane or an `alive` probe read - never on pane-hash bookkeeping alone, so a dead pane's cosmetic redraws cannot re-surface the same death once per recheck window.
Secondmate agent-process liveness (a live pane holding a bare shell) stays owned by the session-start sweep and never wakes the primary from this path, so an ambiguity there can never spawn a duplicate supervisor mid-flight.
Quiet-but-alive declared pauses keep the absorb-and-long-recheck behavior unchanged; the death wakes add no noise to healthy waits.
...
That declared pause holds even while the crew's own no-mistakes run-step is still actively running: both signals are positive not-wedged evidence, so such a crew parks on the pause cadence instead of treadmilling one wedge escalation per threshold window (the 2026-07-16 residual treadmill fix).
The authoritative current-state verdict still ejects the park for every non-absorbable result, so a failed or gate-parked run behind a stale `paused:` line surfaces immediately, and a busy pane keeps outranking the declared pause.
A crew whose reconciled `bin/fm-crew-state.sh` verdict is `done` is an absorbable park the same way, but only behind positive park-anchor evidence: its declared `paused:` last status line or an armed per-task check script such as the merge poll (the 2026-07-16 treadmill fix - such a crew owes nothing until its PR merges or closes).
A finished crew with no such anchor, and every failed, gate-parked, ambiguous, or errored crew-state read, still surfaces immediately.
...
`bin/fm-crew-state.sh`'s header owns the exact state mapping, evidence-freshness order, CI-monitor interpretation, exact-run same-pane recovery exception, and backend/pane/log fallbacks.
The fleet snapshot requests that current-state read whenever task metadata records a target even when exact-home endpoint ownership is `unknown`, so current-state evidence remains available while the separate endpoint field stays inconclusive.
...
On every verified primary harness, tracked hook integration gives the primary session the push-based durable-supervision backstop owned by [turnend-guard.md](turnend-guard.md).
The guard is scoped out of secondmate homes and crewmate/scout worktrees.
...
The daemon escalates captain-relevant events, confirmed crew deaths (a stale reason carrying the watcher's `endpoint-gone`/`agent-dead` verdict, escalated directly rather than re-absorbed as a pause or aged as a wedge), plus a bounded recheck for a declared pause that remains idle, as one batched, single-line digest prefixed with an in-band sentinel marker so firstmate can tell daemon injections apart from real messages.
Its supervisor injection path supports tmux, herdr, zellij, Orca, and cmux endpoints, with `FM_SUPERVISOR_BACKEND` and `FM_SUPERVISOR_TARGET` resolved independently from the task-spawn backend.
Pane existence, busy checks, composer checks, capture, and verified submit route through `bin/fm-backend.sh`; `docs/configuration.md` "Away-mode supervisor backend" owns explicit target requirements and ambient discovery.
Composer-content classification has one shared owner, `bin/fm-composer-lib.sh`, used by tmux, herdr, zellij, Orca, and cmux after each adapter performs its own capture and composer-row recognition.
The daemon injects only into an affirmatively `empty` composer, so both `pending` and `unknown` defer and a bare dead-shell prompt cannot receive an escalation; the complete policy is in [Composer-emptiness safety](herdr-backend.md#composer-emptiness-safety-2026-07-10-fleet-wide-across-all-five-backends).
Any backend outside the verified five refuses at daemon startup.
...
The deeper agent-process liveness probe is separate from that busy-state poll: tmux and herdr have verified classifiers, used by the session-start secondmate recovery sweep and by the watcher's bounded paused-recheck death probe, while zellij, Orca, and cmux currently report `unknown` rather than guess - and an `unknown` never counts as dead, so those backends simply keep the plain absorb behavior.
...
Orca is experimental and selected only explicitly: Orca owns both worktree and terminal lifecycle and records `orca_worktree_id=` plus `terminal=`, while current teardown fails closed at the exact-home duplicate-inventory boundary documented in `docs/orca-backend.md`.
...
`config/backlog-backend` is inherited too; absent or `tasks-axi` selects the default tasks-axi backlog backend, while `manual` suppresses capability reporting without disabling validated handoff delegation and leaves its narrow no-wrapper, single-owner fallback owned by `docs/configuration.md`.
...
Task-scoped notes use `bin/fm-backlog.sh show <id> --full` followed by `bin/fm-backlog.sh update <id> --body-file <path>`, adding `--archive-body` when the prior body should remain recoverable.

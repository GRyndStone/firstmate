...
| Send + submit atomically | `herdr pane run <pane> <command>` | Runs and submits a command in one call; used for the fixed spawn-time commands (`treehouse get`, the `GOTMPDIR` export, and the `NO_MISTAKES_RUN_AGENTS` export) exactly where tmux used one `send-keys ... Enter` call. |

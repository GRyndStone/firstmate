...
Pi keeps a transient latch around its synchronous SDK call, while OpenCode records every SDK flight durably before invoking the client; every callback still runs the predicate before consulting that delivery state.
...
A bounded local wait returns control while a process-retaining retry timer observes a hung flight without issuing another SDK request, and the unresolved flight's token, session, generation, and pending record remain durably quarantined across process or session-lock handoff.
A new process never reissues a quarantined flight because the prior SDK request may still land; only the process that owns the promise may clear the flight after a definitive resolution or rejection.
Any newer blind generation is persisted as the desired handoff while that flight remains active and can be promoted only after the owning process records the old flight's definitive rejection.
...
An explicitly resolved `promptAsync` promise is queue acknowledgement, which is persisted before pending or flight cleanup so startup can finish deletion without redelivery.
OpenCode distinguishes owned, other-owned, and unknown session-lock probes; unknown ownership mutates no shared handoff state and retains a bounded process-local retry until ownership becomes knowable.
...
A passive adapter rejects every symlinked path component from the selected Firstmate home through its effective state, handoff directory, and handoff records instead of following it outside that home.
...
`tests/fm-turnend-guard.test.sh` covers the shared predicate, primary scoping, `FM_HOME` and `FM_STATE_OVERRIDE` precedence, active away-mode daemon provenance plus exact endpoint validation through agent-process or affirmative `empty`/`pending` composer evidence including rejection of a surviving bare-shell pane, a foreground checkpoint that is live at the first Stop and dead at the retry, exact bounded-detail omission disclosure, repeated Pi, OpenCode, and Grok continuation delivery while blindness persists, bounded OpenCode prompt hangs, durable cross-process flight quarantine, durable acknowledged cleanup, unknown lock-probe retry ownership, real Pi void-return acknowledgement, Pi session-lock ownership, linked-crewmate recovery isolation, final and ancestor symlink rejection across passive handoff adapters, volatile persistence and acknowledged-cleanup retry ownership, fail-closed guard-launch and handoff-preparation errors, exact Grok session identity, concurrent Grok Stop serialization, live-owner token binding, hook-exit, readiness, signal-reaping, and per-session barriers, dependency-free fail-closed behavior without `jq` or valid input, tracked hook registration for all five harnesses, and the Grok adapter's permission-mode regression.

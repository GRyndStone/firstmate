...
The primary project hook runs `bin/fm-turnend-guard-grok.sh`, which forces one same-session follow-up via `grok --resume` for every Stop event that would still end blind.

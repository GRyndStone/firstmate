...
obs_scorable() {
  ...
  jq -cn \
    ...
    '{
      ...
      binding:{id:"w",remaining:$R,T:$T,window_seconds:$W},
      diagnostics:[]
    }'
}
...
test_surplus_and_pressure() {
  ...
  # Clear history so prior/history do not dominate; R=100 => prior used=0 => B=0, S=R.
  ...
  jq -e '.S == 100 and .scorable == true and .pressure > 1 and .score > 100' \
    <<< "$near" >/dev/null || fail "near-expiry surplus should raise pressure: $near"
  jq -e '.S == 100 and .pressure >= 1 and .score >= 100' \
  ...
  pass "projected surplus S and expiry pressure behave as specified"
}
...
test_history_learns_burn_rate() {
  ...
  # Two samples: remaining dropped 20 over 100s => B=0.2
  fm_usage_burn_history_record claude w 80 1000
  fm_usage_burn_history_record claude w 60 1100
  obs=$(obs_scorable claude 60 5000 18000)
  scored=$(fm_usage_burndown_score_one "$obs")
  B1=$(jq -r '.B' <<< "$scored")
  # 0.2 * 5000 = 1000 > R=60 => S=0
  jq -e '.B_source == "history" and .B > 0.19 and .B < 0.21 and .S == 0' \
  ...
  pass "feasible burn B is learned from observed consumption history"
}

test_multi_select_prefers_expiring_surplus() {
  local profiles scored_obs selection
  # High remaining far from reset with high burn (S~0) vs lower remaining near reset with zero burn (high S).
  # Claude: R=90, T large relative, high history burn.
  printf '%s\n' '{"samples":[]}' > "$FM_USAGE_BURN_HISTORY"
  fm_usage_burn_history_record claude w 100 1000
  fm_usage_burn_history_record claude w 10 1100   # B = 90/100 = 0.9 /s
  # Codex: no history, R=50, near expiry T=300, W=18000 => S=50, high pressure
  fm_usage_burn_history_record codex w 50 2000
  fm_usage_burn_history_record codex w 50 2100   # no decrease => B falls to prior/zero
  ...
  assert_contains "$(jq -r '.explain' <<< "$selection")" "highest expiry-weighted surplus" \
    "explain must name the burndown reason"
  pass "multi-select minimizes projected expired-unused usage via weighted surplus"
}
...
test_posture_boundaries() {
  ...
  jq -e '.posture == "freeze"' <<< "$scored" >/dev/null || fail "10 remaining => freeze: $scored"
  pass "observational postures still change at 60/80/90 percent used"
}

...
#!/usr/bin/env bash
case "${1:-}" in
  kill-window|list-windows|list-panes) exit 0 ;;
  *) exit 1 ;;
esac
SH
...
make_fake_root() {
  ...
  # fm-composer-lib.sh: fm-tmux-lib.sh sources it as a sibling for the shared
  # composer classifier (extracted after the P1 backend split).
  ...
  install_empty_tmux_inventory "$fake/bin"
...
test_teardown_removes_tasktmp_dir() {
  ...
  PATH="$fake/bin:$PATH" FM_HOME="$fake" bash "$fake/bin/fm-teardown.sh" "$id" >/dev/null 2> "$err" \

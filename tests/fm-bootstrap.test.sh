...
unsupported agy xhigh effort is flagged^{"rules":[{"when":"antigravity work","use":{"harness":"agy","model":"gemini-3.1-pro","effort":"xhigh"}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - invalid effort: agy:xhigh
agy baked-effort model is accepted^{"rules":[{"when":"antigravity fallback","use":{"provider":"antigravity","harness":"agy","model":"gemini-3.1-pro-high"}}]}^grep^CREW_DISPATCH: active config/crew-dispatch.json
...
default candidate set with usage-burndown is accepted^{"rules":[],"default":{"use":[{"harness":"claude"},{"harness":"codex"}],"select":"usage-burndown"}}^grep^  default: usage-burndown[claude, codex]

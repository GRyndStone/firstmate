...
You talk to a single agent - the first mate - and it runs the crew for you: spawning autonomous agents in a visible session backend, giving each a clean git worktree, supervising them to completion, and handing you finished PRs, approved local merges, or evidence-backed investigation conclusions.
...
- **Explicit project modes** - each project declares whether its deliverable is a reference (`no-mistakes` or `direct-PR`) or a running local product (`local-first` with a GitHub backup or `local-only`), with an optional `+yolo` autonomy flag.
...
- **Known incorporations** - a declared inventory of what firstmate depends on, checked at session start for published releases nobody installed and for installed tools that quietly stopped honoring something firstmate parses; silent when everything is current, and upgrades stay deliberate, recorded, and reversible.
- **Guarded by construction** - the first mate is read-only over your projects outside guarded clone refreshes, safe branch pruning, and approved local-product fast-forward merges plus local-first backup pushes; crewmates make every project change behind your merge approval.
...
```
 ...
 └───┬────┘   └───┬────┘      └───┬────┘
  ...
  treehouse worktree, Orca worktree, or isolated secondmate home
     ...
     │
     └─ scout: report at data/<id>/report.md ► Firstmate synthesis ► teardown
```

You chat with the first mate.
It routes each request to a crewmate in its own session endpoint and git worktree, supervises the fleet with a zero-token event-driven watcher, and brings you finished PRs, approved local merges, or evidence-backed investigation conclusions.
...
| `/bearings`        | Generate a "pick up where I left off" status report from live fleet evidence - backlog, per-task crew state, open PRs, scout reports, independently analyzed pending decisions, and date-gated queued work - written to a dated file in `data/` and surfaced concisely in chat; read-mostly, mutates no task state |
...
- [docs/state-reconciliation.md](docs/state-reconciliation.md) - durable current-state reconciliation, observable external waits, regression canary, and supervisor rollout.
- [docs/configuration.md](docs/configuration.md) - environment variables, `FM_HOME`, runtime backend selection, optional X mode, the files you set, and harness support.
- [docs/dependency-currency.md](docs/dependency-currency.md) - the declared incorporation inventory, how staleness and broken dependency contracts reach you at session start, and the approved-upgrade and rollback path.

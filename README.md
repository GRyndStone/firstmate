...
| `/bearings`        | Generate a "pick up where I left off" status report from the read-only fleet snapshot - runnable, held, and blocked backlog work, durable program sources, per-task crew state, endpoint anomalies, open PRs, scout reports, pending decisions, and date-gated work - written to a dated file in `data/` and surfaced concisely in chat; read-mostly, mutates no task state |
